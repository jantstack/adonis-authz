...
14. **El catálogo que decide es el de la BD (versión compartida).** `authz_catalog_version` sube como ÚLTIMA
    ...
    ni el memo viejo.
15. **La contención cubre las seis escrituras, contra origen y destino.** `within` vale en
    ...
    hijo ya existe, también su cadena: es un move) y `scopes.detached` (contra el hijo); `requireWithin: true` las
...
16. **`authorizedScopes` es coherente con `authorize` o lanza.** Cada candidato se contrasta con
    ...
    (`ExcludedSubtree`, `expandExcludedSubtrees`). Coste O(descendientes × `resolveAncestors`), acotado por
...
| `src/drivers/database_driver.ts` | Driver SQL propio sobre `authz_*`. `now?` en opciones y `withClock(now)` (2.5 · J1); `whereActive` = `expires_at > now()`; sella `created_at` con el mismo reloj. Verificado en SQLite, PostgreSQL 18 y MySQL 8.4. |
...
| `src/testing/contract.ts` | **El juez**: `runAuthorizationDriverContract` (`level: 'core' \| '2.0' \| '2.1'`, `capabilities` —`listDenies` es par de capacidad solo en `'2.1'` (2E · I5): `true` juzga lo que resta denies y F1, `false` juzga que lo digan con 500 `E_AUTHZ_UNSUPPORTED`; `injectableClock` es par en TODOS los niveles (2.5 · J1): `true` ⇒ tres estados con reloj (core) + caducidad exacta T−1/T/T+1 y `config.clock` (2.1), `false` ⇒ tres estados en tiempo real (1,5 s)—, árbol del harness, `makeTwin?` = otra instancia sobre el mismo backend con otro memo del catálogo; default `twinOf`). Los casos `'2.1'` construyen un `AuthorizationManager` sobre el driver del harness (`managerOver`); el espía Proxy usa `receiver` para ver la vista de `withAncestorsResolver`. Casos de motor (2.5 · J3: identidad como cadena y mayúsculas que no se cruzan; milisegundos y 2040) y de concurrencia (2.5 · J4: dos grants, purga vs grant, sync vs authorize). Se publica en `./testing` para drivers de terceros. `scope_tree.ts`: `memoryScopeTree`/`resolveAncestorsFrom`/`descendantsFrom` (desde `edges()`). |
...
| `providers/`, `services/main.ts`, `configure.ts`, `stubs/` | Wiring Adonis: provider, singleton, `node ace configure`, plantillas publicadas (`config/authorization` cablea `scopes.resolveAncestors` y la misma función a ambos drivers; `config/app_acl`; migración con las cinco tablas `authz_*` + `authz_catalog_version` sembrada). Decisiones de motor en la migración (2.5 · J3, observadas en PG/MySQL): `holder_uuid`/`scope_uuid` `varchar(64)` (PG `uuid` rechazaba ids válidos), `collate 'utf8mb4_bin'` en identidad y slugs (MySQL `*_ci` fundía `abc`/`ABC`), `expires_at` `DATETIME(3)` (MySQL `TIMESTAMP(0)` redondea al segundo y muere en 2038). El espejo es `tests/helpers/schema.ts`. |
...
| `tests/` | `helpers/app.ts` (**harness multi-motor**, 2.5 · J2: `TEST_DB=sqlite\|sqlite-file\|pg\|mysql`; PG/MySQL crean `authz_test_<8 hex>` desde una conexión administrativa y la borran en `teardown()` —también el proceso hijo de `load_without_sdk`—; `sqlite-file` = fichero en `mkdtemp` con pool 2..5 y WAL), `clock` (grep: solo `src/clock.ts` lee la hora), `contract.spec.ts` (database siempre —y «database (sin listDenies)», la cara `listDenies: false` del juez—; openfga si `OPENFGA_TEST_URL`; `injectableClock: true` en ambos), `contract_harness` (conteo de casos del juez, hoy 36 core / 49 en 2.0 / 65 en 2.1 con `listDenies: true` e `injectableClock: false` (68 con el reloj; 59 sin `listDenies`): tocarlo al añadir casos), `manager` (incluye el grupo «lote 2B»: actor, aviso opt-in, composición de `authorizeMany`/`effectivePermissions`/`authorizedScopes` con drivers falsos; los de 2D: F5, F8, F9 —con reloj inyectado y `Date.now` hacia atrás, 2E · H3/I2—, F10, espía que sobrevive a `withAncestorsResolver`; y H1 en `manager.scopes`), `middleware`, `database_driver`, `openfga_driver` (unitarios sin servidor; `authorizeMany` en un batchCheck con dedupe), `spies` (coste por operación: catálogo una vez + 1 revalidación por pregunta, 1 batchCheck, `forRequest`, `authorizeMany`, `effectivePermissions` ≤ 2 lecturas), `catalog_cache` (contrato del memo y de la versión compartida: `withAuthzCatalogWrite`, bump sin trx ⇒ 500, fila ausente ⇒ 503, reloj monótono, `catalog`+`catalogRevalidate` ⇒ 500), `memoize_ancestors`, `hierarchical_resolver` (B4, F6), `sql_descendants` (B2, G1, sobre `demo_scopes` de `helpers/schema.ts`; ciclo con cota > 1000 —MySQL—), `purity` (reglas 1–3 + carga de `index.ts` con el SDK bloqueado, `helpers/load_without_sdk.ts`), `prune_stores`, `configure` (los stubs compilan, con el subpath mapeado), `migration_stub` (seis tablas + semilla + decisiones J3 en el texto del stub; introspección portable con `columnsInfo`), `scope_tree`. |

## Comandos

```bash
npm test               # Japa; SQLite en memoria (pool 1/1), sin app anfitriona
npm run test:sqlite-file   # SQLite en fichero (mkdtemp) con pool 2..5: concurrencia real
...
- Fases: **0** ✅ (juez con `level`/`capabilities`, `ContractScopeTree`, CI con 2.º OpenFGA) →
  ...
  **2.5** ✅ infraestructura de test (J1 reloj inyectable + par `injectableClock`; J2 harness `TEST_DB` con PG 18 /
  ...
  hint de profundidad—; J4 concurrencia; J5 job `engines` en CI; informe `fase-2.5-informe.md`; pendiente el commit
  del dueño) →
