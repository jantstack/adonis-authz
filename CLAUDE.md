...
14. **El catálogo que decide es el de la BD (versión compartida).** `authz_catalog_version` sube dentro de la
    transacción de `syncAuthzCatalog`; el memo del catálogo la contrasta antes de servir (default `'always'`,
    un SELECT por PK por pregunta; `{ everyMs }` es una ventana acotada opt-in) y **nunca sirve una decisión
    con una versión distinta de la de la base**. Un sync en otro proceso se ve en la siguiente pregunta de
    todos; quien escribe `authz_*` a mano llama a `bumpAuthzCatalogVersion()`. Sin fila de versión legible ⇒
    503, nunca el memo viejo.
15. **La contención cubre las seis escrituras.** `within` vale en `grant`/`revoke`/`deny`/`removeDeny` (contra
    el scope), `scopes.attached`/`moved` (contra el padre) y `scopes.detached` (contra el hijo); `requireWithin:
    true` las exige todas y `'non-root'` rechaza además `within: APP_SCOPE` (`E_AUTHZ_WITHIN_ROOT_FORBIDDEN`).
...
16. **`authorizedScopes` es coherente con `authorize` o lanza.** Cada candidato se contrasta con
    `resolveAncestors` (el deny se aplica por su cadena; si no cuelga del scope concedente ⇒ 503
    `E_AUTHZ_RESOLVER_FAILED`); la cota se corta antes de pasear; `excludedSubtrees` son subárboles
    (`ExcludedSubtree`, `expandExcludedSubtrees`). Una vista de `forRequest()` caduca para leer (`maxAgeMs`,
    default 30 s ⇒ 500 `E_AUTHZ_VIEW_EXPIRED`).
...
| `src/manager.ts` | Fachada; resuelve driver del config, dispara `onWrite`. `forRequest({ maxAgeMs = 30_000 })` ⇒ `AuthorizationView` (mismo API; lecturas con ancestros memoizados, escrituras en fresco — 2A; tras `maxAgeMs` las lecturas son 500 `E_AUTHZ_VIEW_EXPIRED` — 2D). Primitivas 2B como composición: `isWithin`/`within` en las SEIS escrituras (`ScopedWriteOptions`; `requireWithin: true \| 'non-root'`), `actor`/`requireActor` (`WriteOptions`), `authorizeMany` (delega —validando el `boolean[]`— o compone), `listDenies`, `effectivePermissions` (`rolesInChain` + `listDenies` una vez), `authorizedScopes` (`none|some|all{excludedSubtrees: ExcludedSubtree[]}`, `descendantsOf` del config, pertenencia por `resolveAncestors`, cotas que cortan antes de pasear y por llamada solo bajan), `expandExcludedSubtrees`. `driver()` = salida documentada de las barreras. Aviso de seguridad opt-in una vez por config. |
...
| `src/define_config.ts` | `defineConfig`/`AuthorizationConfig`: `default`, `drivers`, `holderTypes`, `scopes.resolveAncestors` (la costura del árbol hacia arriba) + `scopes.descendantsOf?`/`maxScopes`/`maxDescendants` (2.1, hacia abajo, solo `authorizedScopes`/`expandExcludedSubtrees`), `requireWithin` (`boolean \| 'non-root'`)/`requireActor`/`warnOnOptInSecurity` (2.1), `catalogs`, `hooks.onWrite`. |
...
| `src/catalog.ts` | `syncAuthzCatalog` (prune de vínculos por defecto; colisiones también contra la BD; 503 con la BD caída; sube `authz_catalog_version` dentro de su transacción e invalida el memo del proceso al terminar), `diffAuthzCatalog`/`runCatalogDiff`/`syncCatalogs` (catálogos disjuntos o 422 `E_AUTHZ_CATALOG_CONFLICT`). |
| `src/catalog_cache.ts` | **Memo del catálogo** (2A; versión compartida 2D · F1): `CatalogCache` (carga perezosa de `authz_*`, 3 consultas con deadline; nunca hechos ni decisiones), revalidación contra `authz_catalog_version` por `view()` (`revalidate: 'always' \| { everyMs }`; recarga si la fila va por delante; sin fila legible ⇒ 503), `readAuthzCatalogVersion()`/`bumpAuthzCatalogVersion({ client? })` (canal entre procesos), `invalidateAuthzCatalog()` (contador del proceso), `invalidate()` de instancia por generación (una carga en vuelo no la pierde). Ambos drivers lo usan (`catalog`/`catalogRevalidate`) tomando UNA foto por operación; `purgeScope` lee SQL en fresco. Caso negativo fijado: escribir `authz_*` sin subir la versión no se ve. |
...
| `src/hierarchical_resolver.ts` | `hierarchicalScopeResolver({ parentOf, maxDepth = 64 })` (2.1, B4): resolutor de ancestros desde un padre (`undefined` = desconocido ⇒ `null`; `null`/`app` = raíz); clave de visitados con separador `\u001f`; padre mal formado (`{app, uuid}`…) ⇒ 503 `E_AUTHZ_RESOLVER_FAILED`; ciclo ⇒ 422 `E_AUTHZ_SCOPE_CYCLE`; profundidad ⇒ 500 `E_AUTHZ_SCOPE_TOO_DEEP` (nunca cadena corta); un throw se propaga. |
...
| `src/errors.ts` | Todos con `status` + `code`: `AuthorizationBackendError` (503) y su subclase `…TimeoutError`; `ScopeResolverError` (503); `InvalidIdentityError`, `InvalidSlugError`, `UnknownScopeError`, `NoScopeResolverError`, `UnknownRoleError`, `UnknownPermissionError`, `CatalogConflictError`, `ScopeCycleError`, `NotWithinError`, `WithinRequiredError`, `WithinRootForbiddenError`, `ActorRequiredError`, `TooManyScopesError` (422); `StoreNotEmptyError` (409); `AuthorizationConfigError`, `AuthorizationInternalError`, `RoleIsNotAccessError`, `PurgeIncompleteError`, `UnsupportedOperationError` (`E_AUTHZ_UNSUPPORTED`), `NoDescendantsResolverError`, `UnsupportedDialectError`, `ScopeTooDeepError`, `ViewExpiredError` (500). Tabla en el README. |
| `src/middleware/app_access_middleware.ts` | `appAccess({ permission })` a nivel `app`. Solo `permission`: `{ role }` ⇒ 500 con receta. |
| `src/testing/contract.ts` | **El juez**: `runAuthorizationDriverContract` (`level: 'core' \| '2.0' \| '2.1'`, `capabilities`, árbol del harness, `makeTwin?` = otra instancia sobre el mismo backend con otro memo del catálogo; default `twinOf`). Los casos `'2.1'` construyen un `AuthorizationManager` sobre el driver del harness (`managerOver`); el espía Proxy usa `receiver` para ver la vista de `withAncestorsResolver`. Se publica en `./testing` para drivers de terceros. `scope_tree.ts`: `memoryScopeTree`/`resolveAncestorsFrom`/`descendantsFrom` (desde `edges()`). |
...
| `providers/`, `services/main.ts`, `configure.ts`, `stubs/` | Wiring Adonis: provider, singleton, `node ace configure`, plantillas publicadas (`config/authorization` cablea `scopes.resolveAncestors` y la misma función a ambos drivers; `config/app_acl`; migración con las cinco tablas `authz_*` + `authz_catalog_version` sembrada). |
...
| `tests/` | `contract.spec.ts` (database siempre; openfga si `OPENFGA_TEST_URL`), `contract_harness` (conteo de casos del juez, hoy 36 core / 49 en 2.0 / 60 en 2.1: tocarlo al añadir casos), `manager` (incluye el grupo «lote 2B»: actor, aviso opt-in, composición de `authorizeMany`/`effectivePermissions`/`authorizedScopes` con drivers falsos; y los de 2D: F5, F8, F9, F10, espía que sobrevive a `withAncestorsResolver`), `middleware`, `database_driver`, `openfga_driver` (unitarios sin servidor; `authorizeMany` en un batchCheck con dedupe), `spies` (coste por operación: catálogo una vez + 1 revalidación por pregunta, 1 batchCheck, `forRequest`, `authorizeMany`, `effectivePermissions` ≤ 2 lecturas), `catalog_cache` (contrato del memo y de la versión compartida), `memoize_ancestors`, `hierarchical_resolver` (B4, F6), `sql_descendants` (B2, G1, sobre `demo_scopes` de `helpers/schema.ts`), `purity` (reglas 1–3 + carga de `index.ts` con el SDK bloqueado, `helpers/load_without_sdk.ts`), `prune_stores`, `configure` (los stubs compilan, con el subpath mapeado), `migration_stub` (seis tablas + semilla), `scope_tree`. |
...
- Fases: **0** ✅ (juez con `level`/`capabilities`, `ContractScopeTree`, CI con 2.º OpenFGA) →
  ...
  **2** primitivas (**2A** ✅ optimización: memo del catálogo, 1 batchCheck —4,33 → 2,03 ms p50—,
  ...
  informe `fase-2-lote-d-informe.md`; pendiente el commit del dueño) →
