...
- **Todo en un driver o todo en otro.** Cada driver es completo por sí mismo (hechos + árbol
  ...
  **La dirección DB → FGA existe desde 3b-3a**: `authz:reconcile --to=openfga [--dry-run] [--prune]`
  ...
  pasada, `manager.freeze()`: escrituras 503 `E_AUTHZ_FROZEN` **reintentable**, lecturas normales y un
...
- **La migración NO es sin pérdida, y las pérdidas se DECLARAN** (3b-3b · B3; panel 2, requisito 3
  ...
  **`runMigrationContract`**, publicado en `/testing` junto al resto de la suite porque un driver de
  ...
  las DOS caras: una respuesta que cambia sin pérdida declarada que la explique **falla**; y **todo
  lo que la pasada dejó fuera y no está declarado falla también** —sin esa mitad un driver podría
  saltarse lo que quisiera mientras las 448 respuestas de ESA siembra no cambiaran, que es justo la
  clase de bug que una siembra fija no ve—; y una pérdida declarada que NUNCA ocurre falla igual (es
  una línea falsa en el README). **Del par del paquete la lista declarada es UNA: `expired`.** Las
