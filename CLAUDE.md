...
- **Todo en un driver o todo en otro.** Cada driver es completo por sí mismo (hechos + árbol
  ...
  **La dirección DB → FGA existe desde 3b-3a**: `authz:reconcile --to=openfga [--dry-run] [--prune]`
  ...
  pasada que ESCRIBE, `manager.freeze()`: escrituras 503 `E_AUTHZ_FROZEN` **reintentable**, lecturas normales y un
  `finally` que descongela pase lo que pase (`withFrozenWrites`); **`--dry-run` NO congela** (3b-6): el
  ...
  job; la flota entera el día que el freeze sea durable).
