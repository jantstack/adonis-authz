...
18. **Un rol local solo existe dentro de su owner** (3B). `authz_roles.owner_scope_key` es `global` (catálogo
    ...
    **El nivel de un rol local nunca está POR ENCIMA de su owner** (3E · P1): si `scopeType` es el nivel de un
    ...
    tiene que aparecer de verdad bajo el owner en el árbol de hoy. **Los globales GANAN y nada es silencioso** (3E · P1 b / P6): `syncAuthzCatalog` ya NO aborta por un
    ...
    (`assignableAtViolations`, sin borrarlos: lo asignado sigue concediendo); las dos cosas son deriva en
...
| `commands/` | `authz:catalog:sync` (`--keep-links`; avisa de `shadowedByGlobal`/`assignableAtViolations`, 3E), `authz:catalog:diff` (exit 1 si hay deriva), `openfga:provision`, `openfga:import` (`--dry-run`, `--reconcile`, `--prune`). Los `openfga:*` importan de `src/openfga.ts`. |
