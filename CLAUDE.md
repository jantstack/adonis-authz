...
- **Todo en un driver o todo en otro.** Cada driver es completo por sí mismo (hechos + árbol
  ...
  **La dirección DB → FGA existe desde 3b-3a**: `authz:reconcile --to=openfga [--dry-run] [--prune]`
  ...
  es la salida humana y `--dry-run` no lanza, lo marca). **`--to=database` es el 3b-3b** y hoy
  responde 500 `E_AUTHZ_UNSUPPORTED` nombrando el método del puerto que falta (`reconcile` es
  opcional; `database` no lo implementa a propósito: sus tablas son el ORIGEN).
