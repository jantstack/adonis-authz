...
18. **Un rol local solo existe dentro de su owner** (3B). `authz_roles.owner_scope_key` es `global` (catálogo
    ...
    **La identidad del rol es el uuid también en el PUERTO** (3D · M1): `RoleQuery` admite `{ uuid }` en
    ...
    hechos de TODOS los homónimos del scope exacto. La unicidad `(slug, nivel)` por cadena se DEFIENDE (3D · M2):
    `withAuthzCatalogWrite` bloquea la fila de `authz_catalog_version` (PG/MySQL; SQLite ya serializa) y
    `defineScopedRole` re-comprueba la colisión dentro de esa transacción leyendo la BASE; `authz:catalog:diff`
    reporta los homónimos visibles como deriva (`ambiguousRoles`, exit ≠ 0). **Un rol cuyo owner sale del árbol no
    sobrevive** (3D · M4): `scopes.detached` purga los roles cuyo owner es ese scope ANTES de los hechos (un driver
    sin `purgeRole` lo dice con 500 sin tocar nada) y ese `(slug, nivel)` vuelve a estar libre. `owner_scope_key`
    solo puede ser `global` o `<tipo>|<uuid>` de un scope que no es la raíz: `app` a mano es catálogo corrupto (500).
