...
18. **Un rol local solo existe dentro de su owner** (3B). `authz_roles.owner_scope_key` es `global` (catálogo
    ...
    **La identidad del rol es el uuid también en el PUERTO** (3D · M1): `RoleQuery` admite `{ uuid }` en
    ...
    NO escribe el catálogo, no mide rango, no enumera el subárbol y no devuelve nada. El rol sobrevive en
    `authz_roles` sin conceder, sin ser membresía y sin poder asignarse (la regla única de visibilidad no cambia);
    lo único que ocupa es su `(slug, nivel)` allí donde todavía se le vea, y `deleteScopedRole` no lo alcanza (422
    `E_AUTHZ_UNKNOWN_SCOPE`: resuelve el owner en fresco). La salida es `authz:catalog:prune-orphans`
    (`manager.pruneOrphanRoles`; `--dry-run` por defecto, `--force` para borrar): operación de PLATAFORMA, sin
    actor ni rango, que lee la BASE (no el memo con ventana), purga con `purgeRole` y notifica `role_purged` en
    orden estable por uuid. **Por qué se quitó de `detached`**: era una escritura de CATÁLOGO al final de una
    ...
    decide nada. La colisión de `defineScopedRole` no cambia y no hacía falta tocarla: bloquea solo el homónimo
...
| `commands/` | `authz:catalog:sync` (`--keep-links`; avisa de `shadowedByGlobal`/`assignableAtViolations`, 3E), `authz:catalog:diff` (exit 1 si hay deriva; `--fail-on-shadows` cuenta además los ensombrecidos, 3G · X3), `authz:catalog:prune-orphans` (3b-0 · Z2: lista los roles locales huérfanos; borra con `--force`), `openfga:provision`, `openfga:import` (`--dry-run`, `--reconcile`, `--prune`). Los `openfga:*` importan de `src/openfga.ts`. |
