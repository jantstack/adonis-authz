...
**Two rules about that transaction (2.4.0-alpha.2, lot L-1).** First, **`{ transaction }` carries the write, never the authority**: the freeze barrier that precedes every write (row `id = 2` of `authz_catalog_version`, see [the freeze](#migrating-and-verifying-authzreconcile-23)) is read through the engine's own connection, **never through your transaction** — a transaction opened *before* a freeze does not see it in its snapshot (InnoDB's REPEATABLE READ, PostgreSQL under that level, SQLite in WAL), and a barrier decided by the caller's snapshot is not a barrier (the suite reproduces the bypass on all three engines and pins the 503). The price is declared: **`{ transaction }` requires a connection pool of at least 2**. With a pool of 1 (SQLite `:memory:`) the barrier cannot get a connection while you hold the only one, and the notification is **503 `E_AUTHZ_BACKEND_TIMEOUT` at `freezeTimeoutMs`** (config, default 5000 ms) — fail-closed, never a hang, never a bypass. Second, `sqlScopeOutbox` **checks whose transaction it is**: it must be an *open* Lucid transaction (`isTransaction === true`) of the **outbox's own connection** (`connection`, or the primary one). A transaction of another connection, a `QueryClient` (`db.connection()`) or the whole `db` service are **500 `E_AUTHZ_CONFIG`** naming both connections, before any statement — until L-1 a transaction of another connection silently enqueued in *its* database (where no relay reads and `dead()` never shows it), and `db` passed the duck-check and wrote outside any transaction, in silence. The same check (`assertCallerTransaction`) is the rule for every future `{ transaction }` of the package.

### Writing facts inside your transaction (2.4.0-alpha.2, lot L-3)

`grant`/`revoke`/`deny`/`removeDeny` with **`{ transaction: trx }`** on the `database` driver write the fact **inside your open Lucid transaction** (`db.transaction()` of the driver's connection — the primary one): commit and the fact is there with your rows, roll back and neither is. That is exactly what `transactionalWrites: true` promises — *both or neither* — and the published suite pins it **by census**, not by the answer: after a rollback `authz_assignments`/`authz_denies` hold zero rows for that holder, for the four writes, on SQLite (file), PostgreSQL and MySQL (`runAuthorizationDriverContract`, the `true` face; a `transactions` harness hook lets a third-party driver open its own transaction and count its own rows).

**What goes through your transaction: the write, and only the write** — the `INSERT`/`UPDATE`/`DELETE`, plus the "does it already exist?" read that belongs to it (a `grant` and its re-grant inside the same transaction see each other). **What never does: the authority** — the freeze barrier, the catalog and `resolveChain` are read through the engine's own connection while you hold yours. Hence the deployment rule: **`{ transaction }` requires a pool of at least 2**. With a pool of 1 (SQLite `:memory:`) the barrier cannot get a connection and the write is 503 `E_AUTHZ_BACKEND_TIMEOUT` at `freezeTimeoutMs`, with **zero statements** on your transaction — fail-closed, never a hang, never a bypass; a pool-of-1 deployment declares `new DatabaseAuthorizationDriver({ transactionalWrites: false })` and gets the immediate 500 `E_AUTHZ_UNSUPPORTED` of gate 1 instead. While the engine is **frozen**, `{ transaction }` is the retryable 503 `E_AUTHZ_FROZEN` before the first statement on your transaction. The transaction must be *yours and the driver's*: another connection, a `QueryClient` (`db.connection()`) or the whole `db` are 500 `E_AUTHZ_CONFIG` naming both connections, before any statement.

**What changes when the write lives as long as your transaction — measured, per engine.** Two `grant`s of the same fact in two open transactions:

| engine | the second `INSERT` | error you get | your transaction afterwards |
|---|---|---|---|
| PostgreSQL | waits on the unique index until the first transaction ends; on its commit, `23505` | **409 `E_AUTHZ_WRITE_CONFLICT`** ("roll back and retry") | **aborted** (`25P02` on every later statement until you roll back) |
| MySQL | waits (shared lock on the duplicate record) until the first ends; `1062` | **409 `E_AUTHZ_WRITE_CONFLICT`** | alive: only the statement was undone |
| SQLite (file, WAL) | does not wait: a reader cannot become the writer while another holds the reserved lock (the busy handler is skipped there to avoid a deadlock) — `SQLITE_BUSY` at once, *before* the first commits | 503 `E_AUTHZ_BACKEND_UNAVAILABLE` | alive |

And when the driver's `timeoutMs` elapses while your statement waits: PostgreSQL cancels the query (`pg_cancel_backend`) and leaves your transaction **aborted**; MySQL kills it (`KILL QUERY`) and your transaction lives on — both are 503 `E_AUTHZ_BACKEND_TIMEOUT` with `onWrite` `indeterminate: true, transactional: true`; SQLite's synchronous driver cannot elapse a deadline and you get the `SQLITE_BUSY` above. One more SQLite fact: a transaction that *read* before another writer committed (a freeze put and lifted after your first `SELECT`, say) cannot write afterwards (`SQLITE_BUSY` snapshot) — the engine's rule, classified as 503. In every case the driver never issues a second statement on a transaction that may be aborted: the choice between roll back and retry is yours, and `onWrite` publishes nothing for the loser (a UNIQUE clash is not a deadline).

The same port exists on the **relations** side since lot L-4 — `relate`/`unrelate`/`purgeObject`/`purgeSubject` with `{ transaction }` on the `database` relations driver, same rule, same guard, same pool requirement, plus what is specific to tuples (the partition trigger inside your transaction, expiry renewal rolling back to the previous expiry, the deadlock of two crossed writes as 409) — see [Relations](#relations-rebac-24).

**Known limit (the `{trx}` panel, §6.2).** The *decision* that authorises the write is not in your transaction even though the row is: `resolveChain` reads your tree through your `db`, so creating a scope and granting on it inside the same transaction is 422 `E_AUTHZ_UNKNOWN_SCOPE` — fail-closed. The way out is a `ScopeChainResolver` that closes over your transaction (the package's own suite does that); the recipe by direction is in the docs lot. The catalog API (`defineScopedRole`/`updateScopedRole`/`deleteScopedRole`) refuses `{ transaction }`: it is the cross-process serializer.
...
your open Lucid transaction** of the driver's connection — the same rule, the same guard and the same
deployment requirement as [facts](#writing-facts-inside-your-transaction-240-alpha2-lot-l-3): *the write
goes through your transaction; the authority never does* (the freeze barrier, F-05 and `assertWrite` are
decided before the first statement on it — the suite spies on the transaction and counts zero), **pool ≥ 2**
(a pool-of-1 deployment declares `new DatabaseRelationsDriver(config, { transactionalWrites: false })` and
gets gate 1's immediate 500), and *yours and the driver's* (another connection, a `QueryClient` or the whole
`db` are 500 `E_AUTHZ_CONFIG` before any statement). With a transaction present `relate` no longer opens its
own: the check-then-insert runs on yours, so **the partition trigger fires inside your transaction** (a
cross-partition userset row your own code inserts on that transaction is refused by the engine there; on
PostgreSQL that leaves your transaction aborted and the next `relate` on it is a classified 503, on MySQL and
SQLite it lives on — measured), and **renewing an expiry (delete+insert) rolls back to the previous expiry**:
the census after the rollback holds the old row with its old `expires_at` and its old uuid. The published
suite pins the four writes **by census** (`runRelationsDriverContract`, the `true` face: rollback ⇒ zero new
tuples in the partition, `purgeObject`/`purgeSubject` delete and revert *together*; a `transactions` harness
hook lets a third-party driver open its own transaction and count its own rows). Two open transactions
writing the same **userset** tuple: PostgreSQL and MySQL make the second wait on the unique index and hand
it **409 `E_AUTHZ_WRITE_CONFLICT`** when the first commits (PostgreSQL aborts it, MySQL only undoes the
statement); SQLite (file) answers `SQLITE_BUSY` at once (503). Two transactions writing two tuples in
...
`relate`/`unrelate`/`purge*` **landed with lot L-4** (above); on `openfga` it is the honest 500.
